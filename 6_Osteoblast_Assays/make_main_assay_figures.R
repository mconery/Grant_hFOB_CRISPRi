...
#UPDATED: Now includes both Benjamini-Hochberg (p.adj.BH) and Bonferroni (p.adj.bonf) corrections
...
#UPDATED: Now includes both correction methods

correct_hmsc_bh_bonf <- function(hmsc_comparison_df, treat = "BMP2"){
...
hmsc_alp_comparison_df <- correct_hmsc_bh_bonf(hmsc_alp_comparison_df)

hmsc_alp_count_comparison_df <- correct_hmsc_bh_bonf(hmsc_alp_count_comparison_df)

hmsc_alp_intensity_dapi_comparison_df <- correct_hmsc_bh_bonf(hmsc_alp_intensity_dapi_comparison_df)

#Call the function on the hMSC ARS Results

hmsc_ars_comparison_df <- correct_hmsc_bh_bonf(hmsc_ars_comparison_df)

hmsc_ars_count_comparison_df <- correct_hmsc_bh_bonf(hmsc_ars_count_comparison_df)

hmsc_ars_intensity_dapi_comparison_df <- correct_hmsc_bh_bonf(hmsc_ars_intensity_dapi_comparison_df)

#Call the function on the hMSC Adipo Results

hmsc_adipo_comparison_df <- correct_hmsc_bh_bonf(hmsc_adipo_comparison_df, treat = "Adipo")

hmsc_adipo_count_comparison_df <- correct_hmsc_bh_bonf(hmsc_adipo_count_comparison_df, treat = "Adipo")

hmsc_adipo_intensity_dapi_comparison_df <- correct_hmsc_bh_bonf(hmsc_adipo_intensity_dapi_comparison_df, treat = "Adipo")
...
#UPDATED: Now includes separate functions for BH and Bonferroni corrections

append_signif_BH <- function(plot_df, comparison_df){
  ...
  signif_sirnas <- comparison_df[comparison_df$p.adj.BH <= 0.05,"group1"]
...
append_signif_Bonf <- function(plot_df, comparison_df){
  ...
  signif_sirnas <- comparison_df[comparison_df$p.adj.bonf <= 0.05,"group1"]
...
#Call the functions on all the dataframes for BH correction

hfob_alp_filt_BH <- append_signif_BH(hfob_alp_filt, hfob_alp_comparison_df)

hfob_alp_count_filt_BH <- append_signif_BH(hfob_alp_count_filt, hfob_alp_count_comparison_df)

hfob_alp_intensity_dapi_BH <- append_signif_BH(hfob_alp_intensity_dapi, hfob_alp_intensity_dapi_comparison_df)

hmsc_alp_filt_BH <- append_signif_BH(hmsc_alp_filt, hmsc_alp_comparison_df)

hmsc_alp_count_filt_BH <- append_signif_BH(hmsc_alp_count_filt, hmsc_alp_count_comparison_df)

hmsc_alp_intensity_dapi_BH <- append_signif_BH(hmsc_alp_intensity_dapi, hmsc_alp_intensity_dapi_comparison_df)

hmsc_ars_filt_BH <- append_signif_BH(hmsc_ars_filt, hmsc_ars_comparison_df)

hmsc_ars_count_filt_BH <- append_signif_BH(hmsc_ars_count_filt, hmsc_ars_count_comparison_df)

hmsc_ars_intensity_dapi_BH <- append_signif_BH(hmsc_ars_intensity_dapi, hmsc_ars_intensity_dapi_comparison_df)

hmsc_adipo_filt_BH <- append_signif_BH(hmsc_adipo_filt, hmsc_adipo_comparison_df)

hmsc_adipo_count_filt_BH <- append_signif_BH(hmsc_adipo_count_filt, hmsc_adipo_count_comparison_df)

hmsc_adipo_intensity_dapi_BH <- append_signif_BH(hmsc_adipo_intensity_dapi, hmsc_adipo_intensity_dapi_comparison_df)

#Call the functions on all the dataframes for Bonferroni correction

hfob_alp_filt_Bonf <- append_signif_Bonf(hfob_alp_filt, hfob_alp_comparison_df)

hfob_alp_count_filt_Bonf <- append_signif_Bonf(hfob_alp_count_filt, hfob_alp_count_comparison_df)

hfob_alp_intensity_dapi_Bonf <- append_signif_Bonf(hfob_alp_intensity_dapi, hfob_alp_intensity_dapi_comparison_df)

hmsc_alp_filt_Bonf <- append_signif_Bonf(hmsc_alp_filt, hmsc_alp_comparison_df)

hmsc_alp_count_filt_Bonf <- append_signif_Bonf(hmsc_alp_count_filt, hmsc_alp_count_comparison_df)

hmsc_alp_intensity_dapi_Bonf <- append_signif_Bonf(hmsc_alp_intensity_dapi, hmsc_alp_intensity_dapi_comparison_df)

hmsc_ars_filt_Bonf <- append_signif_Bonf(hmsc_ars_filt, hmsc_ars_comparison_df)

hmsc_ars_count_filt_Bonf <- append_signif_Bonf(hmsc_ars_count_filt, hmsc_ars_count_comparison_df)

hmsc_ars_intensity_dapi_Bonf <- append_signif_Bonf(hmsc_ars_intensity_dapi, hmsc_ars_intensity_dapi_comparison_df)

hmsc_adipo_filt_Bonf <- append_signif_Bonf(hmsc_adipo_filt, hmsc_adipo_comparison_df)

hmsc_adipo_count_filt_Bonf <- append_signif_Bonf(hmsc_adipo_count_filt, hmsc_adipo_count_comparison_df)

hmsc_adipo_intensity_dapi_Bonf <- append_signif_Bonf(hmsc_adipo_intensity_dapi, hmsc_adipo_intensity_dapi_comparison_df)
...
raw_intensity_df_BH <- rbind.data.frame(
  ...
  hfob_alp_filt_BH %>% dplyr::select(siRNA, Plate, Replicate, signif, Value) %>% dplyr::mutate(subplot = "hFOB ALP"),
  
  hmsc_alp_filt_BH %>% dplyr::filter(Treatment != "CON") %>% dplyr::select(siRNA, Plate, Donor, signif, Value) %>% dplyr::rename(Replicate = Donor) %>% dplyr::mutate(subplot = "hMSC ALP"),
  
  hmsc_ars_filt_BH %>% dplyr::filter(Treatment != "CON") %>% dplyr::select(siRNA, Plate, Donor, signif, Value) %>% dplyr::rename(Replicate = Donor) %>% dplyr::mutate(subplot = "hMSC ARS"),
  
  hmsc_adipo_filt_BH %>% dplyr::filter(Treatment != "CON") %>% dplyr::select(siRNA, Plate, Donor, signif, Value) %>% dplyr::rename(Replicate = Donor) %>% dplyr::mutate(subplot = "hMSC Adipo")
...
count_df_BH <- rbind.data.frame(
  ...
  hfob_alp_count_filt_BH %>% dplyr::select(siRNA, Plate, Replicate, signif, Value) %>% dplyr::mutate(subplot = "hFOB ALP"),
  
  hmsc_alp_count_filt_BH %>% dplyr::filter(Treatment != "CON") %>% dplyr::select(siRNA, Plate, Donor, signif, Value) %>% dplyr::rename(Replicate = Donor) %>% dplyr::mutate(subplot = "hMSC ALP"),
  
  hmsc_ars_count_filt_BH %>% dplyr::filter(Treatment != "CON") %>% dplyr::select(siRNA, Plate, Donor, signif, Value) %>% dplyr::rename(Replicate = Donor) %>% dplyr::mutate(subplot = "hMSC ARS"),
  
  hmsc_adipo_count_filt_BH %>% dplyr::filter(Treatment != "CON") %>% dplyr::select(siRNA, Plate, Donor, signif, Value) %>% dplyr::rename(Replicate = Donor) %>% dplyr::mutate(subplot = "hMSC Adipo")
...
intensity_dapi_df_BH <- rbind.data.frame(
  ...
  hfob_alp_intensity_dapi_BH %>% dplyr::select(siRNA, Plate, Replicate, signif, Value) %>% dplyr::mutate(subplot = "hFOB ALP"),
  
  hmsc_alp_intensity_dapi_BH %>% dplyr::filter(Treatment != "CON") %>% dplyr::select(siRNA, Plate, Donor, signif, Value) %>% dplyr::rename(Replicate = Donor) %>% dplyr::mutate(subplot = "hMSC ALP"),
  
  hmsc_ars_intensity_dapi_BH %>% dplyr::filter(Treatment != "CON") %>% dplyr::select(siRNA, Plate, Donor, signif, Value) %>% dplyr::rename(Replicate = Donor) %>% dplyr::mutate(subplot = "hMSC ARS"),
  
  hmsc_adipo_intensity_dapi_BH %>% dplyr::filter(Treatment != "CON") %>% dplyr::select(siRNA, Plate, Donor, signif, Value) %>% dplyr::rename(Replicate = Donor) %>% dplyr::mutate(subplot = "hMSC Adipo")
...
raw_intensity_df_Bonf <- rbind.data.frame(
  ...
  hfob_alp_filt_Bonf %>% dplyr::select(siRNA, Plate, Replicate, signif, Value) %>% dplyr::mutate(subplot = "hFOB ALP"),
  
  hmsc_alp_filt_Bonf %>% dplyr::filter(Treatment != "CON") %>% dplyr::select(siRNA, Plate, Donor, signif, Value) %>% dplyr::rename(Replicate = Donor) %>% dplyr::mutate(subplot = "hMSC ALP"),
  
  hmsc_ars_filt_Bonf %>% dplyr::filter(Treatment != "CON") %>% dplyr::select(siRNA, Plate, Donor, signif, Value) %>% dplyr::rename(Replicate = Donor) %>% dplyr::mutate(subplot = "hMSC ARS"),
  
  hmsc_adipo_filt_Bonf %>% dplyr::filter(Treatment != "CON") %>% dplyr::select(siRNA, Plate, Donor, signif, Value) %>% dplyr::rename(Replicate = Donor) %>% dplyr::mutate(subplot = "hMSC Adipo")
...
count_df_Bonf <- rbind.data.frame(
  ...
  hfob_alp_count_filt_Bonf %>% dplyr::select(siRNA, Plate, Replicate, signif, Value) %>% dplyr::mutate(subplot = "hFOB ALP"),
  
  hmsc_alp_count_filt_Bonf %>% dplyr::filter(Treatment != "CON") %>% dplyr::select(siRNA, Plate, Donor, signif, Value) %>% dplyr::rename(Replicate = Donor) %>% dplyr::mutate(subplot = "hMSC ALP"),
  
  hmsc_ars_count_filt_Bonf %>% dplyr::filter(Treatment != "CON") %>% dplyr::select(siRNA, Plate, Donor, signif, Value) %>% dplyr::rename(Replicate = Donor) %>% dplyr::mutate(subplot = "hMSC ARS"),
  
  hmsc_adipo_count_filt_Bonf %>% dplyr::filter(Treatment != "CON") %>% dplyr::select(siRNA, Plate, Donor, signif, Value) %>% dplyr::rename(Replicate = Donor) %>% dplyr::mutate(subplot = "hMSC Adipo")
...
intensity_dapi_df_Bonf <- rbind.data.frame(
  ...
  hfob_alp_intensity_dapi_Bonf %>% dplyr::select(siRNA, Plate, Replicate, signif, Value) %>% dplyr::mutate(subplot = "hFOB ALP"),
  
  hmsc_alp_intensity_dapi_Bonf %>% dplyr::filter(Treatment != "CON") %>% dplyr::select(siRNA, Plate, Donor, signif, Value) %>% dplyr::rename(Replicate = Donor) %>% dplyr::mutate(subplot = "hMSC ALP"),
  
  hmsc_ars_intensity_dapi_Bonf %>% dplyr::filter(Treatment != "CON") %>% dplyr::select(siRNA, Plate, Donor, signif, Value) %>% dplyr::rename(Replicate = Donor) %>% dplyr::mutate(subplot = "hMSC ARS"),
  
  hmsc_adipo_intensity_dapi_Bonf %>% dplyr::filter(Treatment != "CON") %>% dplyr::select(siRNA, Plate, Donor, signif, Value) %>% dplyr::rename(Replicate = Donor) %>% dplyr::mutate(subplot = "hMSC Adipo")
...
# Apply factor level reset for both correction methods
...
intensity_plot_BH <- ggplot(raw_intensity_df_BH, aes(x = siRNA, y = Value, colour = signif)) +
  ...
  scale_color_manual(values = c("gray", "dodgerblue3", "#48494B"), labels=c("Not Significant     ", "Adj. P-Value < 0.05 (BH)     ", "Not Tested     "),
...
intensity_plot_Bonf <- ggplot(raw_intensity_df_Bonf, aes(x = siRNA, y = Value, colour = signif)) +
  ...
  scale_color_manual(values = c("gray", "dodgerblue3", "#48494B"), labels=c("Not Significant     ", "Adj. P-Value < 0.05 (Bonf)     ", "Not Tested     "),
...
#Make combined plot for BH correction (original with updated legend)

tiff(paste0(out_dir, "main_assay_figure.box.BH.tif"), width = 18000, height=6000, res=1000)
...
#Make combined plot for Bonferroni correction (new figure)

tiff(paste0(out_dir, "main_assay_figure.box.Bonferroni.tif"), width = 18000, height=6000, res=1000)
...
# UPDATED: Now includes both BH and Bonferroni corrections in all output tables

### Make a Single Table Containing Raw Intensity Data ###

raw_intensity_df_BH %>% dplyr::mutate(Plate = ifelse(subplot == "hFOB ALP", NA, Plate)) %>%
...
count_df_BH %>% dplyr::mutate(Plate = ifelse(subplot == "hFOB ALP", NA, Plate)) %>%
...
rbind.data.frame(
  ...
  raw_intensity_df_BH %>%
    ...
    dplyr::filter(Plate != "Plate 2" | siRNA != "Control") %>% #Removes duplicate controls in hFOB experiment
    
    dplyr::group_by(siRNA) %>%
    
    dplyr::summarize(mean_value=mean(Value, na.rm = TRUE), reps=n()) %>%
    
    left_join(hfob_alp_comparison_df, join_by(siRNA ==group1, reps==reps)) %>%
    
    dplyr::select(siRNA, reps, mean_value, p, p.adj.BH, p.adj.bonf) %>%
  ...
  count_df_BH %>%
    ...
    left_join(hfob_alp_count_comparison_df, join_by(siRNA ==group1, reps==reps)) %>%
  ...
  intensity_dapi_df_BH %>%
    ...
    left_join(hfob_alp_intensity_dapi_comparison_df, join_by(siRNA ==group1, reps==reps)) %>%
...
rbind.data.frame(
  ...
  raw_intensity_df_BH %>%
    ...
    dplyr::select(siRNA, Plate, reps, mean_value, p, p.adj.BH, p.adj.bonf) %>%
    
    dplyr::mutate(p = ifelse(is.na(p), "Not Tested", as.character(p)), 
                  p.adj.BH = ifelse(is.na(p.adj.BH), "Not Tested", as.character(p.adj.BH)),
                  p.adj.bonf = ifelse(is.na(p.adj.bonf), "Not Tested", as.character(p.adj.bonf))) %>%

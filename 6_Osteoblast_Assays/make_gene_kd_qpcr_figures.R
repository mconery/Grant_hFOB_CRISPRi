...
hmsc_osteo_supplement <- hmsc_osteo_diff %>% dplyr::group_by(siRNA, Gene, Treatment) %>% 
  dplyr::summarize(mean_pct_diff=mean(pct_diff, na.rm = TRUE), reps = n(), p.adj = first(p.adj)) %>% 
  dplyr::select(Gene, Treatment, reps, mean_pct_diff, p.adj) %>% 
...
hmsc_adipo_supplement <- hmsc_adipo_diff %>% 
  ...
  dplyr::summarize(mean_pct_diff=mean(pct_diff, na.rm = TRUE), reps = n(), p.adj = first(p.adj), Gene=first(Gene)) %>% 

...
#make_assay_figures.dapi_normalized.R

#The purpose of this script is to make figures and tables for the hmsc
#cell model assay results using values normalized by cell counts. 
#This script runs locally on the dapi normalized assay results. 
...
hmsc_ars_comparison_df <- hmsc_ars_comparison_df %>% mutate(p.adj = ifelse(is.na(p), NA, 1))
...
#Correct for multiple testing separately for the two test types in the hMSC adipo experiment
hmsc_adipo_comparison_df <- hmsc_adipo_comparison_df %>% mutate(p.adj = ifelse(is.na(p), NA, 1))
...
tiff(paste0(out_dir, "DAPI_normalized.box.tiff"), width = 12000, height=12000, res=1000)
ggplot(combined_plot_df, aes(x = siRNA, y = Value, color=signif)) +
  geom_boxplot(linewidth=1.03, width=0.5) + 
  #  stat_pvalue_manual(combined_comparison_df, x= "siRNA", y = "max_value_padded", label = "p.signif", label.size = 6, color = "signif", show.legend=FALSE) + 
  geom_hline(yintercept = 1, color = "red", linetype = "dashed", linewidth = 1.4, alpha = 0.6) +
...
### hMSC adipo###
hmsc_adipo_temp <- combined_hmsc_adipo_plot %>% dplyr::group_by(siRNA) %>% 
  dplyr::summarize(mean_fold_change=mean(Value)) %>% 
